...
mainGuiControlsInit() {
	...
	placeControl(mainGui, minimizeBtn, , 235, "-Hidden", "s11 Norm", "Minimize")
	minimizeBtn.OnEvent("Click", appHide)
    
	updateBtn := mainGui.Add("Button", "Hidden")
	placeControl(mainGui, updateBtn, , 235, "-Hidden", "s11 Norm", "Update")
	updateBtn.onEvent("Click", appUpdate)

	controlAlignCenter(mainGui, intervalWidth := 10, minimizeBtn, updateBtn)
...
appUpdate(*) {
	Download "https://github.com/Asteos96/GTA-Online-Firewall/releases/latest/download/FwGui_latest.ahk", A_Desktop "\fw_temp.ahk"
	if IniRead(A_Desktop "\fw_temp.ahk", "General", "version") = IniRead(A_ScriptFullPath, "General", "version")
		MsgBox("Версия совпадает")
	else
		MsgBox("Версия не совпадает")
	FileDelete A_Desktop "\fw_temp.ahk"
}
